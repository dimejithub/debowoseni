...
create table if not exists public.newsletter_settings (
  ...
  frequency    text not null default 'off'
               check (frequency in ('off','weekly','biweekly','monthly','quarterly')),
...
-- Re-run safety: if the table already existed from an earlier phase 5, widen
-- the cadence check to include newer options (e.g. quarterly).
alter table public.newsletter_settings drop constraint if exists newsletter_settings_frequency_check;
alter table public.newsletter_settings add constraint newsletter_settings_frequency_check
  check (frequency in ('off','weekly','biweekly','monthly','quarterly'));
