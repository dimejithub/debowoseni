...
export default function JournalPost() {
  ...
  useEffect(() => {
    ...
    getPostBySlug(slug)
      ...
      .catch(() => active && setError("Post not found"));
    getPublishedPosts(6).then((all) => {
      if (!active) return;
      setMore(all.filter((p) => p.slug !== slug).slice(0, 3));
    });
    return () => {
      active = false;
    };
  ...
  if (error) {
    return (
      <div className="container-page py-32 text-center" data-testid="journal-post-missing">
        ...
        <Link
          to="/journal"
          className="mt-8 inline-flex items-center gap-2 rounded-full border border-line bg-surface px-5 py-3 text-sm text-ink hover:border-lime hover:text-lime"
        >
  ...
  return (
    <article className="pt-8" data-testid="journal-post">
      ...
      <header className="container-prose pt-10 pb-10 text-center md:pt-14">
        <Reveal>
          <p className="text-xs uppercase tracking-[0.24em] text-lime">{post.category}</p>
        </Reveal>
        <Reveal delay={0.08}>
          <h1 className="mt-6 font-display">{post.title}</h1>
        </Reveal>
        <Reveal delay={0.16}>
      ...
      <div className="container-page">
        <Reveal className="overflow-hidden rounded-[20px] border border-line">
          <img
            src={post.cover_url || FALLBACK_POST_COVER}
            alt={post.title}
      ...
      <div className="container-prose py-14">
        {post.excerpt && (
          <p className="mb-8 text-lg text-ink/85">{post.excerpt}</p>
        )}
        <div
        ...
        <section className="border-t border-line bg-surface/30 py-20">
          <div className="container-page">
            <div className="mb-10 flex items-end justify-between">
              <h2 className="font-display text-3xl tracking-tight">More from the journal</h2>
            ...
            <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
              {more.map((p) => (
                ...
                >
                  ...
                  <h3 className="font-display text-xl tracking-tight group-hover:text-lime">
                    {p.title}
                  </h3>
                </Link>
              ))}
